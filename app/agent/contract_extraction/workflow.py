...
def build_contract_extraction_graph():
    ...
    contract_overview_subgraph = build_contract_overview_generation_subgraph()
    ...
    async def run_contract_overview_subgraph(
        state: ContractExtractionState,
    ) -> ContractExtractionState:
        """调用建议名称子图，并写回带证据的正式结果。"""
        result = await contract_overview_subgraph.ainvoke(
            {
                "base_context": state["base_context"],
        ...
        contract_overview = result["contract_overview"]
        if contract_overview.status == "failed":
            raise RuntimeError(
                contract_overview.error or "建议名称没有形成可靠结果"
            )
        return {"contract_overview": contract_overview}
    ...
    graph.add_node("contract_overview_generation", run_contract_overview_subgraph)
    ...
    graph.add_edge("classification", "contract_overview_generation")
    graph.add_edge("contract_overview_generation", "assemble_prefill_context")
