...
from app.agent.contract_extraction.subgraph.document_understanding.document_structure.visual_grounding.prompt import (
...
def select_unit_pages(
    ...
    *,
    ...
    if missing_pages:
        raise ValueError(
            f"单元 {unit.unit_id} 引用了不存在的已准备页面 {missing_pages}"
        )
    return (
