...
QUESTION_PROPOSAL_COMMON_PROMPT_VERSION: Final = "retrieval-question-proposal-common-v1"
QUESTION_PROPOSAL_TARGET_PROMPT_VERSION: Final = "retrieval-question-proposal-target-v1"
...
# 字段说明：
# evidence：支持本规划适用于当前合同的简短 PDF 原文证据；仍须回到 PDF 核对，不可把它当成完整答案。
# reasoning_summary：多个指南关注点为何能够由同一个真实用户问题共同覆盖的简洁依据。
# focus_requirement：本次问题必须覆盖的合同对象、范围、条件、时间、金额、责任或例外；它是问题要求，不是预制答案。"""

_QUESTION_PROPOSAL_COMMON_TASK_BASE: Final = """你已获得当前合同的完整原始 PDF、权威文档导航结构和已确认分类结果。你还会收到一份当前问题规划，以及程序根据该规划精确选择的一个或多个权威提问指南关注点。当前任务是把这份问题规划表达为一个贴近真实用户查询的正式问题。
...
4. 规划中的 evidence 用于确认选题和定位；生成问题前仍应核对 PDF。不得把规划证据、推理摘要或指南文字直接拼成僵硬问题。
5. 只询问合同如何约定或哪些合同事实成立，不要求合同外法律分析、效力判断、争议预测或行动建议。

人类提问风格：
1. 模拟采购、销售、项目、财务、法务或管理人员真实查找合同时会输入的表达，不写成审查清单、字段标签、指南复述或法律教材问题。
2. PDF 能够确认时，使用相关方简称、具体产品、服务或项目作为交易锚点；不得猜测提问者属于合同哪一方，也不要机械地以“合同约定的”开头。
...
2. 按“合同证据 → 简洁推理摘要 → 正式问题”的顺序提交。证据应简短且可由 PDF 核对；推理摘要说明最终问题如何忠实覆盖当前规划；question 只提交一个连贯问题。
    ...
    *,
    ...
    target = "\n\n".join(
        (
            ...
            "下一步：核对 PDF，并调用 propose_question 提交忠实覆盖当前问题规划的自然用户问题。",
        )
    )
    messages.append(
        {
