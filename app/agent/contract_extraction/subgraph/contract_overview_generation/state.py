...
class ContractOverviewGenerationModel(BaseModel):
    """建议文件名上下文与结果使用的不可变严格基类。"""

    model_config = ConfigDict(extra="forbid", frozen=True)


class ContractOverviewGenerationContext(ContractOverviewGenerationModel):
...
class ContractOverviewGenerationToolCallAudit(ContractOverviewGenerationModel):
...
class ContractOverviewResult(ContractOverviewGenerationModel):
    """包含建议名称与合同摘要的概览，或不含半成品的技术失败。"""

    status: Literal["generated", "failed"]
    document_id: str = Field(pattern=r"^[0-9a-f]{64}$")
    evidence: tuple[ContractOverviewEvidence, ...] = ()
    reasoning: str | None = None
    file_name: str | None = None
    summary: str | None = Field(default=None, min_length=1, max_length=3000)
    ...
    completion_tokens: int | None = Field(default=None, ge=0)
    cached_tokens: int | None = Field(default=None, ge=0)
    tool_calls: tuple[ContractOverviewGenerationToolCallAudit, ...] = ()
    error: str | None = None
    ...
    def validate_status_payload(self) -> Self:
        ...
        if (
            ...
            raise ValueError("可靠建议文件名必须包含证据、命名理由、最终名称和合同摘要")
        return self


class ContractOverviewGenerationSubgraphState(TypedDict, total=False):
    """在上下文组装与合同概览生成之间传递的子图状态。"""
...
__all__ = [
    ...
    "ContractOverviewGenerationModel",
