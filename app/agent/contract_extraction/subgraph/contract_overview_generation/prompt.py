...
CONTRACT_OVERVIEW_GENERATION_TASK = f"""你已获得当前合同按原始顺序排列的页面图像、权威文档导航结构，以及只保留命名所需信息的合同分类摘要。

任务目标：
为当前合同生成一个准确、简洁、便于用户识别和修改的展示文件名。名称应让用户不打开文件也能大致知道“这是关于什么的哪类合同”，而不是简单抄录缺乏辨识度的原始标题；名称只生成主体，不附加文件扩展名；同时生成供用户理解合同内容的事实摘要。
...
摘要要求：
1. summary 用于让用户快速了解这份合同讲什么。阅读整份合同，以页面为唯一事实来源；文档导航用于定位，分类摘要用于辅助理解，不能据此补充页面未记载的事实。
2. 使用 1～2 个简洁的自然段，不加标题、编号、项目符号或表格。通常一段即可；只有确有必要补充独立的核心范围时才写第二段，不为凑段数重复内容，也不把条款清单改写成长段落。
3. 重点说明合同类型、涉及的项目或标的、核心交易或服务内容，以及各方主要承担什么。主体名称仅在有助于识别合同时保留；不追求信息完整，不展开具体执行细节。
4. 不逐项总结付款、验收、违约责任、争议解决等条款，不罗列金额、日期、技术参数或明细。仅当某项信息直接决定合同主题或核心交易范围时才简要保留，不因原文存在就写入摘要。
5. 已写入的内容必须准确，保留会改变核心含义的条件与范围。不计算原文未给出的数值，不猜测缺失事实，不添加法律评价、风险判断或履约结论。无法确认的次要信息直接省略。
6. summary 是内容概览，不是命名理由或推理过程。完成主题介绍后即停止，不接着补充条款。工具的 3000 字符上限仅是校验边界，不是建议长度。提交时 summary 位于 file_name 之后。

虚构摘要示例（只说明表达方式，不代表当前合同事实）：
- 单一服务合同可写一段：“本合同涉及某园区中央空调的年度维护保养，由服务方提供巡检、故障维修和定期保养，委托方提供现场配合。服务范围包括办公楼及配套机房的空调设施。”无需继续介绍付款比例、验收程序或违约责任。
- 合同同时包含设备采购与独立的配套服务时，可用第一段介绍采购标的与供货分工，第二段概括安装调试及培训范围；不得逐项展开服务条款，也不得在原文没有配套服务时套用这个结构。
...
2. `think` 用于基于页面、文档结构和分类摘要进行名称和摘要的事实核对；可以按需调用，但不得连续调用超过两次。其 `reasoning` 不提交正式名称，名称与摘要均核对完成后应调用终止工具。
3. `submit_contract_overview` 是唯一终止工具，必须按 `evidence → reasoning → file_name → summary` 的顺序提交页面证据、简洁命名理由、唯一建议名称，最后提交合同摘要。
...
def build_contract_overview_generation_messages(
    ...
    if not messages:
        raise ValueError("合同概览生成的合同基础上下文不能为空")
