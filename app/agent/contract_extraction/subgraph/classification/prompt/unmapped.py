...
UNMAPPED_TYPE_DESCRIPTION_PROMPT_VERSION = "unmapped-type-description-v1"

UNMAPPED_TYPE_DESCRIPTION_TASK = """全部正式合同类别均已完成判断，但没有任何类别命中。

请重新核查原始 PDF，用一段简洁中文描述这份合同实际约定的交易类型。描述应直接说明一方主要提供、完成或让渡什么，另一方主要支付、返还或承担什么；不要只改写合同标题，不要创造类别 code，不要把结果命名为“其他”或“未分类”。
