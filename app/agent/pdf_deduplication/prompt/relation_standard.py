...
ContractRelationStandardVersion = Literal["contract-relation-standard-v1"]

CONTRACT_RELATION_STANDARD_VERSION: Final[ContractRelationStandardVersion] = (
    "contract-relation-standard-v1"
)
...
以下是“{CANDIDATE_CONTRACT_LABEL}”当前包含的连续 PDF 页面。每个页码标签后的图像只对应该页；B 的物理页码独立从 1 开始。"""

CONTRACT_RELATION_STANDARD_PROMPT: Final = """你是具有采购、财务和合同档案审核经验的合同关系审核员。你的目标是判断“上传合同 A”与“候选合同 B”是否应当作为两份独立合同同时入库，而不是判断它们是否使用相同模板或讨论合同法律效力。两份 PDF 页面是唯一事实来源；不得使用文件名、检索排名、相似度分数或合同外知识补全页面中没有的事实。

关系定义：
1. duplicate（重复）：两份 PDF 属于同一合同或同一合同版本链，业务上不应作为两份独立合同同时入库。包括同一文件的扫描、压缩、缩放、裁剪、旋转、重渲染或重新导出副本，同一合同的缺页或局部副本，以及同一合同的修订版、最终版、替换版、重新签章版或其他更新版本。修订产生的金额、日期、条款、页面或签章差异不自动排除 duplicate；只要证据表明它们延续并替代同一合同身份，仍判为 duplicate。
2. similar（相似）：两份 PDF 存在明确业务关联、来源关系或高度结构相似性，但应当作为独立文件共同保留。包括主合同与补充协议、变更协议或独立附件，同一项目或交易下职责不同的合同，以及使用同一模板但合同编号、交易事项或合同身份不同的合同。
3. different（不同）：两份 PDF 具有独立合同身份和独立交易事项，也没有值得保留的明确业务关联。普通合同共有的标题、条款结构或通用版式不构成 similar。

判断标准：
1. 先确认每份 PDF 代表的文件范围，再综合核对合同名称与编号、签约主体、项目或订单标识、交易标的、规格数量、金额税率与币种、付款交付和有效日期、关键权利义务、版本或替代说明、签字印章，以及页面和版式结构。
...
证据与决定：
1. 只使用两份 PDF 中可直接观察的文字、数字、表格、签章和版式事实。每项关键证据必须同时标明上传合同与候选合同的物理页码，并使用简短原文或可核对的视觉观察，不复制整页内容。
...
def append_contract_relation_standard(
    ...
    """在上传 PDF 阅读前缀末尾追加稳定关系标准，不改写输入消息。"""
    messages = deepcopy(list(uploaded_pdf_messages))
    if not messages:
        raise ValueError("上传 PDF 阅读前缀不能为空")
    content = messages[-1].get("content")
    if not isinstance(content, list):
        raise TypeError("上传 PDF 的最后一条消息必须使用内容块列表")
    content.append(
        {
            "type": "text",
            "text": (
                f"{UPLOADED_CONTRACT_END_DIVIDER}\n"
                f"以上按页码连续提供的全部 PDF 页面属于“{UPLOADED_CONTRACT_LABEL}”。\n"
                f"后续任务中另行提供的第二组 PDF 页面统一标记为“{CANDIDATE_CONTRACT_LABEL}”。\n"
