...
class RejectionReplyGeneration(BaseModel):
    ...
    message: NonBlankText = Field(max_length=2000, description='直接给用户的简洁中文纯文本。将摘要中明确的内容作为已掌握的文件事实直接陈述，不说“摘要显示”“根据摘要”等内部来源转述，不伪称逐页检查；无法确认的内容仍保留不确定性。无附件、仅寒暄且没有已确认的服务故障或检查未完成时，礼貌回应、简要介绍合同及财务法律业务协助范围并邀请表达需求，不强调处理停止或要求重提。其他情况说明本次无法继续的具体障碍及最少必要调整，并明确调整后重新提问或提交；服务故障则建议稍后重新提交。不凭空增加上传要求，不从本轮没有附件推断旧文件不可用，不承诺未经确认的工具能力。不得暗示本次等待补充后自动续接，不含代码、链接、HTML 或业务分析结论')
...
def validate_rejection_reply(content: str) -> RejectionReplyGeneration:
    ...
    try:
        ...
        return RejectionReplyGeneration.model_validate_json(content)
...
class FileSummaryGeneration(BaseModel):
    ...
    summary: NonBlankText = Field(description="仅依据当前文件全部已提供页面生成的中文内容概述，说明可辨认的主要对象、场景、事项和内容范围，不只描述图片或表格等载体形式；用途未知时仍保留可见事实，仅说明原文件中影响理解的具体限制。不依赖用户问题，不补造对象、背景或用途，不作风险分析或建议。")
...
def validate_file_summary(content: str) -> FileSummaryGeneration:
    """严格读取完整 JSON，不截取代码块、不做类型转换或猜测修补。"""
    try:
    ...
    try:
        return FileSummaryGeneration.model_validate_json(content)
    except ValidationError as exc:
        issues = []
...
class TextBusinessRelevanceGeneration(BaseModel):
    ...
    result: TextBusinessRelevanceDecision = Field(description="related：本轮文字可确认财务、法律、合同或直接关联事项；uncertain：缺少识别业务主题所需的信息，依赖不可见文件或前文；unrelated：明确不属于业务范围。仅缺少回答细节仍为 related；执行故障不是 uncertain。本字段不代表整轮准入结果。")
...
class FileBusinessRelevanceGeneration(BaseModel):
    ...
    result: FileBusinessRelevanceDecision = Field(description='related：名称和摘要足以确认文件属于财务、法律、合同、经营管理或业务设备技术资料；uncertain：主题或用途信息不足、关键冲突无法消除；unrelated：明确属于业务范围之外。仅判断当前文件，不代表所有附件或整轮准入。')
