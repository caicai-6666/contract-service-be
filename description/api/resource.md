...
> **用途：** 本文定义前端根据 Elasticsearch 合同文档中的 `file_uri` 读取 `data/contract` 下正式合同 PDF 的接口。全局鉴权和错误约定见 [API 参考](readme.md)。
...
`file_uri` 直接使用合同 Elasticsearch 文档中的同名字段。当前本地文件协议只接受根相对的单层 PDF 地址，例如 `/14db0d...ad3fe.pdf`；不接受外部 URL、相对路径、子目录、查询参数、片段或目录穿越。
...
接口只负责读取磁盘文件，不查询或修改 Elasticsearch，也不根据展示用 `file_name` 定位文件。文件地址和本地处理版 PDF 的存储契约见[合同 Elasticsearch 文档结构](../architecture/data/contract-elasticsearch-document.md)。
