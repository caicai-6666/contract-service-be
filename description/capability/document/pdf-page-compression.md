...
缩放按上述离散 token 公式进行二分查找，始终保留已经满足预算的比例下界；不能仅用面积比例缩放固定次数后直接渲染。由于 patch 向上取整，旧算法可能在最后一次调整后仍然超限，例如 2480×3508 点页面在 4096 token 预算下得到 4104 token，随后误报页面渲染失败。新算法在缩小时留出微小浮点舍入余量，渲染后仍按实际像素校验预算。比例搜索不增加 PDF 实际渲染次数。

工具自身默认单页预算为 2048；应用调用时使用 MLLM 配置和动态分配的预算，不以工具默认值覆盖应用配置。回归覆盖 JPG 嵌入大页面 PDF、横竖页、极端长宽比及最小预算。
