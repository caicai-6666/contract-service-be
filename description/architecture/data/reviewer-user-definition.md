...
登录接口会把审核用户转换为限时免登码；除健康检查和登录外的接口统一通过 FastAPI 依赖校验免登码并取得审核人身份。合同任务在创建时以该名称记录所有者，后续运行列表、快照、SSE、继续和重试都按所有者隔离。客户端不能自行声明任务所有者。接口契约见[审核用户登录 API](../../api/auth.md)和[合同 API](../../api/contract.md)。

---

## 用户访问边界

所有已登录用户均可查看、新增和删除共享正式合同，以及新增、查看和删除共享注意事项。上传、继续、重试、正式入库和取消提取任务仍校验任务所有者；会话和附件仍按所属用户隔离。凭据缺失、无效或过期返回 `401`。
