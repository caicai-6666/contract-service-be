...
提示词版本为 `contract-document-detection-v3`。模型只被告知已经获得按原始顺序排列的上传文档页面图像，不接收文件格式暗示。法律概念以[《民法典》第四百六十四条](https://www.cac.gov.cn/2020-06/01/c_15925617772683192.htm)关于合同的定义为基础，并转换为项目可执行的文档类型规则；本 Agent 不判断合同成立、生效、可执行或法律效力。
...
工具版本为 `contract-document-detection-tool-v2`，采用 `tool_choice=auto`，并固定按以下顺序提供：
