...
完整字段契约和用户扩展方式见[检索问题指南定义结构](../../data/retrieval-view-definition.md)。`answer/` 目录及其对象契约已经删除，启动加载器只允许根目录包含 `question/`。
...
应用进入检索准备阶段时先报告“正在统计待生成的检索问题数量”。关注点规划尚未完成前不提供总数；`discover_question_focuses` 形成固定规划后，`generate_questions` 报告 `0 / m`，并在每份并发规划形成 `generated` 或 `failed` 终态时依次报告 `k / m`。一个规划最多生成一个正式问题，因此总数采用规划数，而不是只统计成功问题数。

进度达到 `m / m` 只表示问题生成并行节点已经汇总，后续向量化与融合仍属于同一个 `retrieval_preparation` 阶段，最终以 `stage.completed` 为准。观察性进度不进入问题规划工作区、模型上下文、工具审计或正式检索结果。
