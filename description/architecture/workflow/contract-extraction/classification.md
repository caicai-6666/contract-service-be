...
并发分类直接复用同一个 `ClassificationContext`、相同工具定义和稳定工具顺序。页面已由前置结构理解请求完成媒体缓存填充，分类请求只发送 UUID 引用；vLLM 仍在模型侧复用公共 token 前缀。隔离 A/B 实验表明显式单 token 分类预热只带来很小的计算 token 节约，未形成墙钟收益，因此分类子图不再额外发送预热请求。媒体引用的并发与失效边界见[vLLM 多模态媒体引用](../../../capability/infrastructure/vllm-media-reference.md)。
...
应用执行器启动分类子图时先报告“正在统计待识别的合同类别数量”。`classify_contract` 读取不可变类别目录后报告真实类别总数，并在每个并发类别形成 `matched`、`not_matched` 或 `failed` 终态时把完成数加一。进度依次表现为 `0 / m`、`1 / m`，直至 `m / m`；未映射类型的兜底描述不增加类别总数。
...
分类节点读取启动阶段按[合同交易类别定义结构](../../data/contract-category-definition.md)建立的 `ContractCategoryCatalog` 内存快照，并为每个类别并行发起一次独立判断。每次请求只注入当前类别对象中的权威定义、positive 正例和 negative 反例，判断合同是否具备当前类别要求的完整核心结构；类别卡片只用于边界校准，不能覆盖权威定义。
