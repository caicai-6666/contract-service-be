# 文档结构理解子图的结构发现与视觉定位节点

> **用途：** 本文定义直接消费 `PreparedPDF` 的文档结构理解子图中，单元发现与后置视觉定位节点的边界。
...
本节点不执行完整 OCR，不提取正式字段，不逐条拆分法律条款，也不生成供检索使用的最终合同视图。它产生的页面地图供字段、条款和检索视图子图共同使用，减少各分支对合同整体结构的重复判断和方向偏差。
...
`discover_document_units` 使用异步 `AsyncOpenAI` 适配器执行受工具状态机约束的单元发现循环，并生成不含坐标的语义结构。`locate_document_units` 随后为所有单元并发建立独立工具会话，把完整定位结果写入 `DocumentStructureMetadata.unit_locations`。发现与定位的详细审计分别保留在文档结构理解子图私有的 `unit_discovery` 和 `unit_grounding` 状态中。

---

## 单元发现工具

工具定义位于 `app/agent/contract_extraction/subgraph/document_understanding/document_structure/tool.py`。它们只属于文档结构理解子图的结构发现节点，不从子图包或合同抽取 Agent 的 `__init__.py` 导出，外部服务也不应直接调用。
...
四个函数工具均使用 `strict: false`，配合 `tool_choice="auto"` 绕过本地 vLLM 的 XGrammar 结构化解码路径。程序不会把 `auto` 产生的普通文本当作结构化结果：若一轮没有恰好一个工具调用，原始普通文本仅截断保存在私有审计中，短期记忆临时加入该响应和不回显原文的 XML 格式纠正要求；连续两次纠正机会仍失败时显式终止。正常工具调用仍使用拒绝额外字段的 Pydantic 契约校验参数，并校验页码、非空文本、页面边界顺序和完成覆盖等业务语义。协议、Schema、页码和状态错误共用临时纠错范围；某个动作通过全部校验后立即删除整条失败轨迹，再保留正确动作。`finish` 只是终止请求，程序仍检查至少一个有效单元和全部物理页面跨度覆盖，再决定是否真正退出。
...
节点只读接收异步 PDF 准备服务生成的 `PreparedPDF`。物理页码、PDF 页面尺寸、旋转角度、文本层状态、渲染尺寸、缩放比例和视觉 token 均由程序生成，模型不得重复推断或覆盖。具体页面事实见[PDF 准备服务与文档结构理解子图](document-understanding.md)。
...
合同主题是面向结构导航的简短概览，不替代检索视图生成子图。建议契约如下：
