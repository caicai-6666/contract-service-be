...
> **当前状态：** 可读性之后已接入逐文件并发命名与摘要；相关性分支只接收轻量文字输入。四个相关性维度均已接入约束解码和有限纠错；上下文由服务端选取最新摘要之后最多五轮有效任务。文件维度按最佳有效业务结果计分，执行失败优先，摘要失败时停止。
...
`app.agent.contract_communication.business_gate` 为用户问题、文件及任务准入条件校验提供独立子图。当前实现内存文件打开、页面渲染和视觉可读性判断，由 Communication 正式执行器在 SSE 激活后调用；不影响合同提取与入库，尚未接入智能体骨架。
...
有文件时顺序为 `file_readability → summarize_files → initialize_business_gate → 相关性并行分支 → aggregate_relevance`。聚合通过后结束门禁并允许服务继续；可读性 rejected/failed、摘要未完成以及聚合未放行，统一进入 `reject_request → END`，不再继续摘要或相关性检查。无文件时直接进入 initialize_business_gate，跳过摘要。内部空任务也经聚合进入拒绝出口，不默认放行。文件子图内部检查状态与逻辑保持不变，详见[文件可读性检查子图](file-readability.md)。
...
异步调用使用 `await subgraph.ainvoke({"files": files})`。构建函数返回已编译子图，由正式 Communication 服务复用执行，各轮状态独立；核心问答尚未接入。
...
当前版本为 `file-summary-v2`，权威提示词位于 `app/agent/contract_communication/business_gate/prompt/file_summary.py`。`build_file_summary_messages(rendered_file)` 接收一份已渲染文件，返回准备发送的消息；请求由 `node.py` 中的 `generate_file_summary` 发起。规则覆盖实际内容命名、整份文件总体概述、非合同材料、跨页正文与附件、缺失信息、冲突及空白文件；不注入本轮问题或历史，不进行风险分析。
...
本次只调整摘要任务后缀及 Schema 字段说明，公共页面前缀、三个输出字段、并发与重试逻辑保持不变；没有新增语义质量复核节点或硬性充分性校验。规则能否减少真实模型的内容遗漏仍需图像 PDF 实验验证，不能把提示词要求当作程序保证。
...
同一文件的页面 PNG、媒体 UUID、页码与顺序原样复用，不重新打开、压缩、渲染或提前编码 Base64；只是不同请求重复引用相同页面内容。每次构造独立消息容器，避免并发分支互相改写。实际缓存命中仍依赖模型服务的模板、配置和缓存驻留，目前只验证应用消息前缀一致，未测量真实 vLLM 命中率。摘要执行采用 JSON Schema 约束解码并关闭额外 thinking 通道，与可读性调用方式对齐。
...
每份文件单独构造模型消息、客户端、重试计数和私有审计。`MLLMClient.create_json_chat_completion` 接收三字段 Schema 和 `schema_name=file_summary`，`strict=true`、`enable_thinking=false`；输出 token 上限为配置值与 2048 的较小值。模型服务不支持约束解码时明确 failed，不降级自然语言或工具调用。
...
正式服务经统一拒绝节点收束：SSE thinking → 流式友好提示 → rejected，快照及驻留历史同步，附件不获准落盘。摘要成功仍不等于业务准入；后续适用的相关性分支全部完成后可形成准入判断。核心问答尚未接入；未启用演示时通过后给出未接入提示，不批准附件。启用混合联调时，完整门禁通过后批准附件，并在同轮调用模拟问答，不伪造真实分析；见[混合联调说明](../../../capability/application/communication-ui-demo.md)。
...
| 文件业务相关性 | `unrelated` | 根据文件名称与摘要，本轮文件均未体现与可处理业务范围的联系。 |
...
`node.py` 中 `reject_request_async`（同步适配 `reject_request`）是所有门禁未放行结果的共同出口。`passed` 不进入该节点；可读性熔断、摘要生成或绑定失败、相关性不足、相关性执行失败及内部未完成结果均进入一次，最终统一返回 `status=rejected`。HTTP 认证、请求体校验、主动取消、系统关闭、未捕获的程序异常及第二层执行故障不属于本节点的业务路由；完整门禁通过但未配置核心问答仍保留开发占位提示。
...
- 提示词位于 `prompt/rejection_reply.py`，版本 `rejection-reply-v8`；固定系统规则在前、动态资料在后。明确定位为“本次业务处理已停止后的调整与重新提问指引”，不重新判断准入，不执行原分析任务。系统规则、动态目标与 Schema 同步声明纯文字寒暄的礼貌回应例外，并允许以摘要中的明确内容解释文件类型差异。
...
- 使用已有全局 MLLM 配置、配额和 JSON Schema 约束解码，关闭额外 thinking 通道，输出上限取配置与 2048 token 的较小值。最多尝试 1～3 次，默认 3；只接受正常结束、无工具调用/拒答且通过严格校验的完整响应。
...
正式执行器已将统一拒绝节点的回复接入流式最终提示、`turn.status: rejected`、历史同步与附件清理；全过程展示“正在思考”。检查层仍区分 failed 与 rejected，统一出口保留原始状态并给出不同原因，但对外均表示本轮未获准继续。详见 [门禁拒绝与恢复](../../../api/communication.md#门禁拒绝与恢复)。演示脚本的旧 clarify 场景仅用于界面联调，不作为新的技术门禁实现。
...
passed 是内部业务门禁通过，不等同于合同分析已经完成。有无文件、文字和历史的有效组合均已接通正式判断与聚合；核心问答仍未实现；未启用模拟问答时通过后只说明未接入并以 completed 收束，启用时则批准附件并串接 mock，门禁本身不提前结束轮次。not_implemented 仅保留为内部初始化、空输入或未完成分支的防御性状态，正常有效请求不再因上下文占位停留。
