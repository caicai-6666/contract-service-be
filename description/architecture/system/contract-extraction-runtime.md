...
主 Agent 图可以等待三个下游分支全部结束后统一合并，适合实验或批处理。HTTP 服务在创建任务后先判断上传内容是否属于合同；只有合同才执行查重，无候选自动运行合同结构识别，有候选且无重复时等待确认，再复用相同的分类、建议名称和三个业务子图：
...
flowchart TD
    ...
    detection -->|是合同| summary["文件准入与质量检查"]
    summary -->|通过| deduplication["PDF 查重"]
    summary -->|拒绝或技术失败| quality_failed["当前阶段失败，停止处理"]
    deduplication -->|有候选且无重复| pause["返回候选与文件地址<br/>等待确认"]
    deduplication -->|无候选| structure
...
不同合同与并行分支的模型请求由统一客户端执行[全局并发限制](../../capability/infrastructure/model-concurrency.md)：单 worker 默认 MLLM 20、Embedding 10，节点内信号量不能扩大这一总额度。额度只约束在途模型请求，不限制注册表任务数量或等待期间的内存占用。
...
- `prepared_pdf`：处理版 PDF 的身份、大小、逐页 PNG 及物理/像素尺寸，不驻留完整 PDF 字节；
...
- `contract_overview_view`：命名成功后固定保存的名称、理由、页面证据与合同摘要，供 SSE、快照和运行历史恢复；
- 查重结果、候选确认暂停状态与截止时间，以及人工或自动继续时间；
...
运行列表接口读取同一个 `MemoryRunRegistry`，先按当前审核用户筛选任务所有者，再投影仍可恢复任务的 `run_id`、处理版 PDF 元数据、当前建议文件名摘要、时间摘要和独立的粗粒度状态。列表状态只有 `processing` 与 `blocked`：后台拓扑仍会自动推进时为前者；等待候选确认、公共前置阶段失败、全部分支停止或结果等待复核入库时为后者。业务分支部分失败但其他分支仍在运行时保持 `processing`，全部自动执行停止后再变为 `blocked`。非合同与重复拒绝在保留期内也以 `blocked` 返回，仅供恢复查看或主动取消清理，不允许继续、重试或入库；列表可将 `blocked` 展示为“待处理”，实际操作能力必须由单任务快照决定。过期回收、已取消和已入库释放的任务不进入列表；技术失败任务继续保留为 `blocked`，供前端恢复后定位并重试。PDF 元数据直接来自聚合中的 `PreparedPDF`：文件大小使用重新封装后的字节数，页数使用处理版物理页数，封面宽高使用第 1 页压缩图像的像素尺寸，不复制 PDF 或 PNG 字节。创建响应、列表项和单运行快照使用同一个 `document` 结构；建议名称尚未生成时列表字段为 `null`。
...
`document.file_id` 为内存 PDF 读取标识，复用已有 UUID `run_id`：一轮只有一份处理版，不增加第二份文件缓存或独立索引。`get_processed_pdf` 经所有权及生命周期检查后，在聚合锁内取得元数据与页面引用，释放锁并在线程中按需组装 PDF，验证创建时的大小与 SHA-256，再检查任务是否仍有效。资源路由分块输出，不持锁等待客户端；组装结果不回写聚合。取消、过期、入库释放及重启自然使该标识失效，已开始的响应可持有字节至传输结束。入库同样按需组装并验证身份。内存边界见[PDF 按需组装](../../capability/document/pdf-page-compression.md#内存生命周期与按需组装)，接口及前端恢复方式见[内存处理版 PDF 读取](../../api/resource.md#读取提取任务的内存处理版-pdf)。
...
- 运行列表只包含当前审核用户的聚合；
- 单任务快照、内存 PDF 读取、SSE 订阅、取消、查重确认继续和失败阶段重试，都要求当前用户名与聚合所有者一致；
- 所有者不一致时统一抛出与未知、过期任务相同的 `RunNotFoundError`，HTTP 层返回 `404`，避免通过错误差异枚举其他用户的 `run_id`；
- 越权请求在读取状态或执行状态迁移前终止，不能续期、消费暂停点、启动重试或建立事件订阅。

周期清理和查重候选确认暂停到期属于系统内部生命周期操作，不模拟某个用户访问，因此仍可回收所有用户的到期聚合。正式入库接口采用相同的所有权校验，并直接使用当前登录且与聚合所有者一致的审核人名称形成入库审核信息；入库成功后删除该用户的聚合。
...
| `contract_overview_generation` | 生成建议名称与摘要 | 调用合同概览子图，生成名称、理由、证据和内容摘要。 |
...
`contract_document_detection` 的合同判断及文件质量检查质量检查均通过后才开始 `pdf_deduplication`。文件质量检查已调用 MLLM，采用原生思考和强制 JSON Schema，最多三次输出修复；独立收束节点生成通过、业务拒绝或技术失败及反馈，服务层据此发布 SSE 和推进。质量拒绝沿用本阶段失败反馈，但 retryable=false 且接口禁止原文件重试；技术失败保留有限重试。原合同判断与问题依据仍保留。详见[文件质量检查骨架](../workflow/contract-document-detection/readme.md#文件准入与质量检查骨架)。判定为非合同时，该阶段仍为 `succeeded`，运行转为 `not_a_contract`，发布携带紧凑证据与理由的 `run.document_rejected`，查重及全部下游阶段保持 `pending`。模型连接、工具协议等执行故障属于技术失败，阶段为 `failed`，不能伪装成非合同。

`pdf_deduplication` 成功且没有召回候选时自动开始合同结构识别。工作流内部保留完整候选判断，应用层投影 `duplicate | similar | different` 候选、原始 cosine、理由、名称、PDF 地址及负责人。只要哈希一致或模型判重，运行进入 `duplicate_rejected`，发布携带同一候选列表的 `run.duplicate_rejected` 后结束，不建立暂停定时器。快照和事件保留至普通运行 TTL，拒绝终态仍以 `blocked` 进入可恢复列表；继续、重试及入库均返回冲突。SSE 发送终态事件后关闭，已消费终态事件的重连也立即关闭。

无重复但有超过相似度阈值的召回候选时，发布 `run.deduplication_review_required` 并暂停，`can_continue=true`、`continued_at=null`，设置最长 600 秒的固定确认期限。即便模型将候选判为 `different`，仍展示真实判断并等待用户核对；`POST continue` 消费暂停点后进入结构识别。

只有召回候选为空才发布 `run.continued` 自动进入结构识别：`can_continue=false`、`review_expires_at=null`，`continued_at` 为自动推进时间，事件携带空候选结果。对已自动推进的任务调用 continue 返回冲突。
...
| `ready` | 合同概览生成成功，且 Core、Clause 和 Retrieval 三个分支均已形成当前结果。 |
...
合同概览阶段成功时采用同一恢复协议：`stage.completed` 携带 `contract_overview`，其中包含不带扩展名的名称主体、简洁命名理由、页面证据和合同摘要；单任务快照持续保存完整公共投影，运行历史列表只保存名称字符串摘要。完整模型工具审计仍只存在该阶段不可覆盖的内部尝试记录。前端可以用建议名称初始化最终文件名输入，但入库请求提交的用户值始终覆盖自动建议。
...
八个用户业务阶段的技术失败可以在次数限制内重试；文件准入业务拒绝不允许原文件重试。服务从失败点继续，严格复用更早阶段已经成功且仍保存在聚合中的权威结果，不允许回退重跑成功阶段：合同文档判断重试后继续查重；查重重试成功后，重复则终止，有候选则待确认，无候选则自动继续；结构识别重试后继续分类；分类重试直接复用结构结果并在成功后生成建议名称；建议名称重试复用分类后的 `ExtractionContext`，成功后启动三个下游分支；三个下游阶段各自独立重试。

创建请求内的 PDF 格式检查与渲染发生在任务建立前，不属于用户业务阶段，失败时不会产生 `run_id`，因此不能重试。业务阶段只有状态为 `failed`、属于允许重试的技术失败且尝试次数未达上限时才令 `retryable=true`；业务拒绝保持 `retryable=false`。
...
普通处理阶段的到期时间在业务状态更新时向后刷新。有候选待确认的查重暂停点例外：它使用独立的精确定时任务和固定截止时间，最长 600 秒，读取操作不续期。到期后先向现有订阅者发布 `run.expired`，再从注册表移除整个聚合，使上传处理版 PDF、候选引用、渲染页面、私有审计、向量和草稿随引用释放；周期清理器继续作为普通 TTL 和定时任务异常情况下的兜底。
...
正式入库接收用户确认的 `file_name` 和 `summary`，两者去除首尾空白后必须非空；摘要最多 3000 个字符。服务层将 summary 传入 `ContractMetadata`，随 ingesting 元数据原子写入 SQLite，成功后沿用 ready 发布流程。摘要不进入 Elasticsearch；失败重试使用本次请求中的摘要，历史空摘要不自动回填。接口字段见[正式入库合同](../../api/contract.md#正式入库合同)。
