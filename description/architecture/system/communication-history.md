...
- task.progress 只更新即时进度，不从文案推测调用，也不把高频进度写成历史消息。
...
输入保留原文与文件列表。注册时在数据库同级 upload 目录排他保存 UUID PDF，失败只清理本次新建文件，撞名重试；历史返回文件相对地址，不读文件字节。附件资源接口与会话删除后的文件清理仍待独立实现。
...
当前要求单进程、单 worker；已接入[十分钟记忆归档与空闲驱逐](communication-archive.md)，仍没有总驻留容量控制。SSE 的容量限制不等于完整历史容量限制，长期大量会话需要后续增加驻留预算。累计摘要生成、历史压缩、upload 读取授权及文件垃圾回收仍不包含在当前实现中。
